...
const birthdayDate = new Date('2026-06-04T00:00:00').getTime();
...
let activeReasons = reasons;
if (reasonGrid) {
  const imageFolder = reasonGrid.dataset.imageFolder || 'assets';
  const listedImages = (reasonGrid.dataset.images || '')
    .split(',')
    .map((image) => image.trim())
    .filter(Boolean);
  const imageCount = Number.parseInt(reasonGrid.dataset.imageCount || '0', 10);
  const fallbackImages = Array.from({ length: imageCount || 1 }, (_, index) => `${index + 1}.jpg`);
  const reasonImages = (listedImages.length ? listedImages : fallbackImages).map((image) =>
    image.includes('/') ? image : `${imageFolder}/${image}`
  );
  activeReasons = reasons.slice(0, reasonImages.length);

  reasonGrid.innerHTML = activeReasons
    .map((reason, index) => {
...
$('#randomReasonBtn')?.addEventListener('click', () => {
  $('#randomReason').textContent = activeReasons[Math.floor(Math.random() * activeReasons.length)];
});
...
Happy Birthday Nandhuuu You are special in ways you may never fully realize, and you deserve magic, love, and endless happiness in every chapter of your life.`;
...
cutCakeBtn?.addEventListener('click', async () => {
  ...
  cakeStageText.textContent = 'first slice for Nandhuuu 🎉';
